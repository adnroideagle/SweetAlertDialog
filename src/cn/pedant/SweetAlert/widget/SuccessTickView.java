...
public class SuccessTickView extends View {
	...
	private void init() {
		...
		mPaint.setColor(getResources().getColor(
				R.color.ccloudmi_sweetalert_success_stroke_color));
	...
	public void draw(Canvas canvas) {
		...
		if (mLeftRectGrowMode) {
			...
			leftRect.right = (totalW + CONST_LEFT_RECT_W) / 2
					+ CONST_RECT_WEIGHT - 1;
		...
		rightRect.bottom = (totalH + CONST_RIGHT_RECT_W) / 2
				+ CONST_RECT_WEIGHT - 1;
	...
	public void startTickAnim() {
		...
		Animation tickAnim = new Animation() {
			...
			protected void applyTransformation(float interpolatedTime,
				...
				if (0.54 < interpolatedTime && 0.7 >= interpolatedTime) { // grow
																			// left
																			// and
																			// right
																			// rect
					...
					mLeftRectWidth = mMaxLeftRectWidth
							* ((interpolatedTime - 0.54f) / 0.16f);
					if (0.65 < interpolatedTime) {
						mRightRectWidth = MAX_RIGHT_RECT_W
								* ((interpolatedTime - 0.65f) / 0.19f);
					}
					invalidate();
				} else if (0.7 < interpolatedTime && 0.84 >= interpolatedTime) { // shorten
																					// left
																					// rect
																					// from
																					// right,
					...
					mLeftRectGrowMode = false;
					mLeftRectWidth = mMaxLeftRectWidth
							* (1 - ((interpolatedTime - 0.7f) / 0.14f));
					mLeftRectWidth = mLeftRectWidth < MIN_LEFT_RECT_W ? MIN_LEFT_RECT_W
							: mLeftRectWidth;
					mRightRectWidth = MAX_RIGHT_RECT_W
							* ((interpolatedTime - 0.65f) / 0.19f);
					invalidate();
				} else if (0.84 < interpolatedTime && 1 >= interpolatedTime) { // restore
																				// left
																				// rect
																				// width,
																				// shorten
																				...
																				// const
					mLeftRectGrowMode = false;
					mLeftRectWidth = MIN_LEFT_RECT_W
